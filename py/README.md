...
The Python SDK for the Openwhyd API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Authentication()` — each
...
| `lastArtists` | `list` | Recently posted artists |
| `lastFm` | `dict` |  |
| `lnk` | `dict` |  |
...
| `pl` | `list` | User playlists |
| `redirect` | `str` | URL to redirect to |
| `twId` | `str` | Twitter handle |
| `twSec` | `str` | Twitter session secret |
| `twTok` | `str` | Twitter session token |
| `uId` | `str` | ID of new user if successful |
...
```
py/
├── openwhyd_sdk.py         -- Main SDK module
