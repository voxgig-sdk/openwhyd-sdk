...
The Python SDK for the Openwhyd API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Authentication(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
```python
# Create
created, _ = client.Authentication(None).create({"name": "Example"}, None)
...
result, err = client.Openwhyd(None).load(
    {"id": "test01"}, None
)
