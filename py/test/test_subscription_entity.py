    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OPENWHYD_TEST_SUBSCRIPTION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        subscription_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.subscription")))
        subscription_ref01_data = None
        if len(subscription_ref01_data_raw) > 0:
...
def _subscription_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "OPENWHYD_TEST_SUBSCRIPTION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OPENWHYD_TEST_SUBSCRIPTION_ENTID"))
