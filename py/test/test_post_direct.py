...
class TestPostDirect:
    ...
    def test_should_direct_load_post(self):
        setup = _post_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-post", "live" if setup["live"] else "unit")
        if _skip:
