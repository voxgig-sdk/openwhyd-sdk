...
describe('AuthenticationDirect', async () => {
  ...
  test('direct-load-authentication', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-authentication', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPENWHYD_TEST_AUTHENTICATION_ENTID']
