...
describe('GetUserPostDirect', async () => {
  ...
  test('direct-list-get_user_post', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_user_post', setup.live)) return
    if (skipIfMissingIds(t, setup, ["get_user_post01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPENWHYD_TEST_GET_USER_POST_ENTID']
