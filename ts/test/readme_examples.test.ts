...
//     (\`new OpenwhydSDK(...)\` / \`OpenwhydSDK.test(...)\` become
//     \`OpenwhydSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"authentication":{"test01":{"id":"test01"}},"get_user_post":{"test01":{"id":"test01"}},"playlist":{"test01":{"id":"test01"}},"post":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"subscription":{"test01":{"id":"test01"}},"user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenwhydSDK(...)\` and any
// \`OpenwhydSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenwhydSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenwhydSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
