...
class AuthenticationEntity
  ...
  def make
    opts = @_entopts.dup
    AuthenticationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenwhydHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Authentication, Hash] the current Authentication data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OpenwhydHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Authentication fields)
  def match_get
  ...
  #
  # @param reqmatch [AuthenticationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Authentication.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Authentication, Hash] the loaded Authentication; raises OpenwhydError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = OpenwhydHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqdata [AuthenticationCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Authentication, Hash] the created Authentication; raises OpenwhydError on failure
