...
class GetUserPostDirectTest < Minitest::Test
  def test_direct_list_get_user_post
    setup = get_user_post_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-get_user_post", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["get_user_post01"].each do |_live_key|
...
def get_user_post_direct_setup(mockres)
