...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = OpenwhydOmni.make_runner(TEST_JSON_FILE, OpenwhydSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : OpenwhydOmni.errify(err)
    end
