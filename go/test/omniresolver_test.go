...
import (
	...
	omni "github.com/voxgig-sdk/openwhyd-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenwhydError); is {
				out := map[string]any{
					"name":    "OpenwhydError",
...
func omniCtx(arg any, client *sdk.OpenwhydSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenwhydSDK); is {
			client = live
