...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OPENWHYD_TEST_LIVE") == "TRUE" ||
		os.Getenv("OPENWHYD_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OPENWHYD_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.OpenwhydSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.OpenwhydError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.OpenwhydSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.OpenwhydError{Msg: msg, Code: code}
}
