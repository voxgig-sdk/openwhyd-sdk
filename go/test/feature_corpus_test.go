...
//
// NewOpenwhydSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.OpenwhydSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewOpenwhydSDK(opts)
}
...
func fcCandidates(client *sdk.OpenwhydSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.OpenwhydSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.OpenwhydSDK, name string) any {
