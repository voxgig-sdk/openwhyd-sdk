...
func TestPostEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPENWHYD_TEST_POST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func postBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"post01", "post02", "post03", "hot01", "hot02", "hot03", "playlist01", "playlist02", "playlist03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("OPENWHYD_TEST_POST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENWHYD_TEST_POST_ENTID"])
