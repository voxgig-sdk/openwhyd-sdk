...
func TestGetUserPostEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetUserPost(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetUserPost(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getUserPostRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_user_post")))
