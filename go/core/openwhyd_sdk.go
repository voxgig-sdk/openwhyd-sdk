...
func (sdk *OpenwhydSDK) OptionsMap() map[string]any {
...
func (sdk *OpenwhydSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *OpenwhydSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *OpenwhydSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OpenwhydSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *OpenwhydSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *OpenwhydSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("OpenwhydSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *OpenwhydSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OpenwhydSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("OpenwhydSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
