...
module OpenwhydUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = OpenwhydResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenwhydResponse.new(fetched)
    else
