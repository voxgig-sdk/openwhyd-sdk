...
class OpenwhydMakeError
{
    public static function call(?OpenwhydContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OpenwhydContext([], null);
        }
        $op = $ctx->op ?? new OpenwhydOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OpenwhydResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OpenwhydError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OpenwhydError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof OpenwhydError) {
