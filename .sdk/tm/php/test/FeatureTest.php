...
class FtCtrl extends OpenwhydControl
{
...
class FtClient
{
    ...
    private OpenwhydUtility $utility;

    public function __construct(OpenwhydUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenwhydUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenwhydContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenwhydContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenwhydContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenwhydConfig::make_feature($name);
    ...
    private static function buildUrl(OpenwhydSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenwhydError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenwhydResponse($fetched);
        }
    ...
    private function fail(OpenwhydContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenwhydContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
