...
// Openwhyd SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenwhydOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenwhydOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenwhydOmni::structfix($val, Struct::undef()))) {
