...
local GetUserPostEntity = {}
GetUserPostEntity.__index = GetUserPostEntity


function GetUserPostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetUserPostEntity)
  self._name = "get_user_post"
  self._client = client
...
function GetUserPostEntity:get_name()
  return self._name
end


function GetUserPostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetUserPostEntity.new(self._client, opts)
end
...
function GetUserPostEntity:mark_deleted()
  self._deleted = true
end


function GetUserPostEntity:deleted()
  return true == self._deleted
end


function GetUserPostEntity:data_set(args)
  if args ~= nil then
...
function GetUserPostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetUserPostEntity:match_set(args)
  if args ~= nil then
...
function GetUserPostEntity:match_get()
...
function GetUserPostEntity:stream(action, args, callopts)
...
function GetUserPostEntity:_run_op(ctx, post_done)
  local utility = self._utility
