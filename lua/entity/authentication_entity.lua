...
local AuthenticationEntity = {}
AuthenticationEntity.__index = AuthenticationEntity


function AuthenticationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AuthenticationEntity)
...
function AuthenticationEntity:get_name()
  return self._name
end


function AuthenticationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AuthenticationEntity.new(self._client, opts)
end
...
function AuthenticationEntity:mark_deleted()
  self._deleted = true
end


function AuthenticationEntity:deleted()
  return true == self._deleted
end


function AuthenticationEntity:data_set(args)
  if args ~= nil then
...
function AuthenticationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AuthenticationEntity:match_set(args)
  if args ~= nil then
...
function AuthenticationEntity:match_get()
...
function AuthenticationEntity:stream(action, args, callopts)
...
---@param reqmatch AuthenticationLoadMatch
---@param ctrl? table
---@return Authentication
---@return string? err
function AuthenticationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function AuthenticationEntity:_run_op(ctx, post_done)
  local utility = self._utility
