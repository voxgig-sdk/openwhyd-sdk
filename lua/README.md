...
The Lua SDK for the Openwhyd API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Authentication(nil):load({ id = "example_id" }, nil)
...
```lua
-- Create
local created, _ = client:Authentication(nil):create({ name = "Example" }, nil)
...
local result, err = client:Openwhyd(nil):load(
  { id = "test01" }, nil
)
