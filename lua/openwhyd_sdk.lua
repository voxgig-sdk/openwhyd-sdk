...
local OpenwhydSDK = {}
OpenwhydSDK.__index = OpenwhydSDK
...
OpenwhydSDK._make_feature = _make_feature


function OpenwhydSDK.new(options)
  local self = setmetatable({}, OpenwhydSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OpenwhydSDK:options_map()
...
function OpenwhydSDK:get_utility()
  return Utility.copy(self._utility)
end


function OpenwhydSDK:get_root_ctx()
  return self._rootctx
end


function OpenwhydSDK:prepare(fetchargs)
...
function OpenwhydSDK:direct(fetchargs)
...
-- by passing it.
function OpenwhydSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OpenwhydSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OpenwhydSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OpenwhydSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OpenwhydSDK.new(sdkopts)
  sdk.mode = "test"
