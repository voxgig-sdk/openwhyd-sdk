...
boru-driven command-line client **and** interactive REPL for the Openwhyd
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openwhyd-cli)
make build
...
./openwhyd-cli load 1 authentication            # {id:1} shorthand
./openwhyd-cli load '{id:1}' authentication       # explicit match map

# 5. Override the API base URL for a single call
OPENWHYD_BASE=https://api.example.com ./openwhyd-cli load 1 authentication

# 6. No arguments -> interactive REPL
./openwhyd-cli
openwhyd> load 1 authentication
openwhyd> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openwhyd-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/openwhyd-cli load 1 authentication
   ```
...
```sh
./openwhyd-cli load 1 authentication          # scalar shorthand for {id:1}
./openwhyd-cli load '{id:1}' authentication     # explicit match map
```
...
./openwhyd-cli load 1 authentication
```
...
Run with no arguments to open a REPL (prompt `openwhyd>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `authentication`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/openwhyd-cli`. |
...
(`./openwhyd-cli <expr>`) and interactively (the REPL), and expressions compose the
