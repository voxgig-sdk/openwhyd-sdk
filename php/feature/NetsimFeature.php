...
class OpenwhydNetsimFeature extends OpenwhydBaseFeature
{
    ...
    private function _simulate(OpenwhydContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(OpenwhydContext $ctx, array $applied): void
    {
        $client = $this->client;
