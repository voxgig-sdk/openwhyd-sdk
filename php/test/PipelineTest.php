...
{
    private static function utility(): OpenwhydUtility
    {
        return new OpenwhydUtility();
    }
    ...
    private static function ctx(array $over = []): OpenwhydContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenwhydContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenwhydOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenwhydError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenwhydOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenwhydMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenwhydOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenwhydMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenwhydOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenwhydMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenwhydSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenwhydMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenwhydResponse([]),
            'result' => new OpenwhydResult([]),
        ]);
        [, $err] = OpenwhydMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenwhydSpec([]),
            'response' => null,
            'result' => new OpenwhydResult([]),
        ]);
        [, $err] = OpenwhydMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenwhydSpec(['step' => 's']),
            'response' => new OpenwhydResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenwhydResult([]),
        ]);
        [, $err] = OpenwhydMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenwhydSpec(['step' => 's']),
            'response' => new OpenwhydResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenwhydResult([]),
        ]);
        [, $err] = OpenwhydMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenwhydSpec(['step' => 's']),
            'response' => new OpenwhydResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenwhydResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenwhydMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenwhydResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenwhydSpec([]),
            'response' => new OpenwhydResponse([]),
            'result' => new OpenwhydResult([]),
        ...
        [$got, $err] = OpenwhydMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenwhydResult([])]);
        [, $err] = OpenwhydMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenwhydSpec([]), 'result' => null]);
        [, $err] = OpenwhydMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenwhydSpec(['step' => 's']),
            'result' => new OpenwhydResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenwhydOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenwhydMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenwhydSpec(['step' => 's']),
            'result' => new OpenwhydResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenwhydOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenwhydMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenwhydResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenwhydSpec([]),
            'result' => new OpenwhydResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenwhydMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenwhydMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenwhydError('boom', 'boom');
        $utility->fetcher = function (OpenwhydContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenwhydSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenwhydMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenwhydContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenwhydMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenwhydResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenwhydContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenwhydSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenwhydMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenwhydResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenwhydSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenwhydMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenwhydMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenwhydMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenwhydResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenwhydDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenwhydResult(['ok' => false])]);
        $this->expectException(OpenwhydError::class);
        OpenwhydDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenwhydResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenwhydMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenwhydMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenwhydBaseFeature();
        $b = new OpenwhydBaseFeature();
        OpenwhydFeatureAdd::call($ctx, $a);
        OpenwhydFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenwhydBaseFeature
    {
        $f = new OpenwhydBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenwhydFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenwhydContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenwhydSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenwhydPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenwhydResponse(['headers' => null]),
            'result' => new OpenwhydResult([]),
        ]);
        OpenwhydResultHeaders::call($ctx);
