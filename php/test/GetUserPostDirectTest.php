...
class GetUserPostDirectTest extends TestCase
{
    public function test_direct_list_get_user_post(): void
    {
        $setup = get_user_post_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-get_user_post", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["get_user_post01"] as $_liveKey) {
