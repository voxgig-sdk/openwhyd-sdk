...
class PlaylistEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENWHYD_TEST_PLAYLIST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $playlist_ref01_data = null;
        if (count($playlist_ref01_data_raw) > 0) {
...
function playlist_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["playlist01", "playlist02", "playlist03", "username01"] as $k) {
    ...
    $entid_env_raw = getenv("OPENWHYD_TEST_PLAYLIST_ENTID");
