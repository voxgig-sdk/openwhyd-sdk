...
class OpenwhydSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = OpenwhydHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = OpenwhydHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!OpenwhydFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, OpenwhydFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return OpenwhydUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = OpenwhydHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = OpenwhydHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = OpenwhydHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new OpenwhydError($op . "_allow",
                "OpenwhydSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = OpenwhydHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new OpenwhydError("graphql_error",
                "OpenwhydSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
