...
MCP server exposing the Openwhyd SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `openwhyd_list` | `entity`, optional `query` map | First page of records as JSON |
| `openwhyd_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `openwhyd_list` and
`openwhyd_load` tools become available in new sessions.
