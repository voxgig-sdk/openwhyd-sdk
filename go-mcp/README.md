...
[MCP](https://modelcontextprotocol.io) server exposing the Openwhyd SDK as
two agent tools — `openwhyd_list` and `openwhyd_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openwhyd-mcp)
make build

# 2. Provide credentials via the environment
export OPENWHYD_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user openwhyd \
  -- /absolute/path/to/openwhyd-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./openwhyd-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// openwhyd_list: first page of records
{ "entity": "get_user_post" }
{ "entity": "get_user_post", "query": { } }

// openwhyd_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openwhyd-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user openwhyd \
     -- "$PWD"/dist/*/openwhyd-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `openwhyd_list` and `openwhyd_load` tools now appear
   in new sessions. Ask the agent to *"list get_user_post using openwhyd"*
   and it calls `openwhyd_list` with `{"entity":"get_user_post"}`.
...
```sh
export OPENWHYD_APIKEY=sk_live_xxx            # API key
export OPENWHYD_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `openwhyd_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `openwhyd_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENWHYD_APIKEY` | API key sent with every request. |
| `OPENWHYD_BASE` | Optional override of the API base URL. |
...
```sh
./openwhyd-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"openwhyd_load","arguments":{"entity":"authentication","query":{"id":1}}}}'
```
